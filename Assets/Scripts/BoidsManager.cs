...
public class BoidsManager : MonoBehaviour
{
    ...
    [SerializeField] private float Seperation = 1;
    ...
    private void Start()
    {
        ...
        {
            Boids.Add(Instantiate(BoidPrefab, new Vector3(Random.Range(-5f, 5f), Random.Range(-5f, 5f), Random.Range(-5f, 5f)), Quaternion.identity));
        }
    }
    ...
    public void MoveAllBoidsToNewPosition()
    {
        ...
        for (int i = 0; i < Boids.Count; i++)
        {
            ...
            if (distance > BoxSize)
            {
                tmp.velocity = (transform.position - tmp.transform.position).normalized * BorderJump;
            }
            tmp.UpdateBoids();
        }
    }
    ...
    public Vector3 Rule2(BoidObject B)
    {
        ...
        for (int i = 0; i < Boids.Count; i++)
        {
            ...
            if(Boids[i] != null)
            {
                if(Vector3.Distance(Boids[i].transform.position, B.transform.position) < Seperation)
                {
